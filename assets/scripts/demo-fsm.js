...
exports = {
    init: function (target) {
        ...
        model = new State.StateMachine("fsm");
        var initial = new State.PseudoState("init-fsm", model, State.PseudoStateKind.Initial);
        ...
        demo = new State.StateMachineInstance("instance");
        State.initialise(model, demo);
    },
    ...
    _evaluate: function (message) {
        if (evaluating) {
            ...
            setTimeout(function () {
                State.evaluate(model, demo, message);
            }, 1);
            return;
        }
        evaluating = true;
